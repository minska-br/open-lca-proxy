    ...
    for product in products:
        ...
        try:
            ...
            food_calculation.process_calculations.append(
                _calculate_for_product(product_amount = product.amount, process_name = process_name, product_name = product.name)
            )
...
def _find_process(processes: List[Tuple], product_name: str):
    ...
    processes_found_name, match_score = process.extract(product_name, processes_name_list, limit = 10)[-1]
    ...
    processes_found = [item for item in processes if item[0] == processes_found_name][0]
